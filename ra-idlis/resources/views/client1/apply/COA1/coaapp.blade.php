...
<body>
			...
			<div class="col-md-8">
				<div class="card">
					<div class="card-header">
						<div class="row">
							...
							<div class="col-md-3 text-left">
								<h5 class="card-title text-uppercase text-center">CERTIFICATE OF ACCREDITATION</h5>
					...
					<div class="card-body" id="ptcbody">
						...
						<div class="row col-border-right forMFOWS" hidden style="border-top: 1px solid; border-right: 1px solid; border-left: 1px solid; background-color: #bfcddc;">
							<div class="col">
								<p>For Pharmacy</p>
							</div>
						</div>
						<div class="row col-border-right forMFOWS" hidden style="border-top: 1px solid; border-right: 1px solid; border-left: 1px solid;">
							<div class="col-md-12">
								<p>Main Pharmacy:</p>
								<div class="mb-3">
	...
	<script type="text/javascript">
		...
		(function() {
			...
			function fPTCApp() {
				let token = document.getElementsByName('_token')[0], ocid = document.getElementById('ocid'), classid = document.getElementById('classid'), subClassid = document.getElementById('subClassid'), facmode = document.getElementById('facmode'), funcid = document.getElementById('funcid'), aptid = document.getElementById('aptid'), gtype = document.getElementsByName('type'), type = "", ghgpid = document.getElementsByName('hgpid'), gaddon = document.getElementsByName('addon'), gfacid = document.getElementsByName('facid'), uid = document.getElementById('uid'), hfep_funded = document.getElementById('hfep_funded'), mhfep_funded = '0', massignedRgn = assignedRgn, noofmain = document.getElementById('noofmain'), noofsatellite = document.getElementById('noofsatellite');
				...
				let sArr = ['_token='+token.value, 'uid='+uid.value, 'appid='+curAppid, 'ocid='+ocid.value, 'classid='+classid.value, 'subClassid='+subClassid.value, 'facmode='+facmode.value, 'funcid='+funcid.value, 'aptid='+aptid.value, 'type='+type, 'hfep_funded='+mhfep_funded, 'assignedRgn='+massignedRgn, 'noofmain='+noofmain.value, 'noofsatellite='+noofsatellite.value];
				...
				sendRequestRetArr(sArr, "{{asset('client1/request/customQuery/fCOAApp')}}", "POST", true, {
			...
			function findSelName(elName, colName, tblName, insId, arrCol, clId) {
				...
				let facidWithPharmacy = ['MSERV','mfowsRMF'];
				...
				if(dom != null || dom != undefined) {
					...
					if(idom != null || idom != undefined) { if(arr.length > 0) {
						if(arr.length == 1) { arr[0].ischecked = 'checked'; getAssigned(arr[0]['facid']);}
						let apString = "", selString = '<option selected value hidden disabled>Please select</option>';
						for(let i = 0; i < Math.ceil(arr.length/4); i++) {
							if(i == 0){
								if(typeof(arr[0]['facid']) != 'undefined'){
									if($.inArray(arr[0]['facid'], facidWithPharmacy) >= 0){
										$('.forMFOWS').removeAttr('hidden');
									} else {
										$('.forMFOWS').attr('hidden',true);
										$("#noofmain").val('');
										$("#noofsatellite").val('');
									}
								}
			...
			function procChkSelData() {
				if(mappform.length > 0) {
					let mappformArr = ['ocid', 'classid', 'subClassid', 'facmode', 'funcid', 'noofsatellite','noofmain'], chReq = [['ocid', 'mclass', 'classid', ['classid', 'classname'], ['subClassid']], ['isSub', 'msubclass', 'subClassid', ['classid', 'classname'], []], [], [], []], mappformArrChk = ['ocid', 'facmode', 'funcid'], premThis = true;
