                        ...
                        @foreach ($BigData as $data)
                         @php
                          ...
                          @php
                            ...
                            @php continue; @endphp
                            @endif
                            @if(FunctionsClientController::existOnDB('fdaevaluation',[['appid',$data->appid],['requestFrom',$request],['decision','RFC']]))   )
                            @php continue; @endphp
                            @endif
                          @php
                            $status = '';
                          ...
                          <tr>
                            ...
                            <!-- <td class="text-center" style="font-weight:bold;">{{(AjaxController::getTransStatusById($data->FDAstatus)[0]->trns_desc ?? '')}}</td> -->
                              <td><center>
                                  <button type="button" title="Evaluate Payment for {{$data->facilityname}}" class="btn btn-outline-primary" onclick="window.location.href='{{asset('employee/dashboard/processflow/FDA/recommendation')}}/{{$data->appid}}/{{$request}}'"  {{$ifdisabled}}><i class="fa fa-fw fa-clipboard-check" {{$ifdisabled}}></i></button>
                              </center></td>
                          </tr>
                          @endif
                        @endif
