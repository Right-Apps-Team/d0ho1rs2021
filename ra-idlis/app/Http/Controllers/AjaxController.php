<?php 
	...
	class AjaxController extends Controller
	{
		...
		public static function savePosition(Request $request)
		{
			try 
			{
				$updateData = array('posname'=>$request->name);
				...
				switch ($Cur_useData['grpid']) {
					...
					case 'HFERC':
						$anotherData = DB::table('appform')
							...
							->join('hferc_team', 'appform.appid', '=', 'hferc_team.appid')
							->select('appform.*', 'hfaci_serv_type.*','region.rgn_desc', 'x08.authorizedsignature', 'x08.email', 'x08.streetname', 'x08.barangay', 'x08.city_muni', 'x08.province', 'x08.zipcode', 'x08.rgnid as aprgnid', 'appform.rgnid', 'hfaci_grp.hgpdesc', 'city_muni.cmname', 'apptype.aptdesc', 'province.provname', 'barangay.brgyname', 'ownership.ocdesc', 'class.classname', 'trans_status.trns_desc', 'x08.uid')
							->orderBy('appform.appid','desc')
							->distinct()
							->get();
		...
		public static function saveGroupMange(Request $request) // Update
		{
			try 
			{
				$update = array('grp_desc'=>$request->name);
	...
	public static function createMobileSessionIfMobile(Request $request){
		try {
			if(Agent::isMobile() && array_key_exists('isMobile', $request->all()) && array_key_exists('uid', $request->all())){
